...
/**
 * This class stands for listEvents Structs
 * @package ChronoApi
 * @subpackage Structs
 */
class ChronoApiListEvents extends AbstractStructBase
{
    /**
     * The events
     ...
     * - minOccurs: 0
     * - nillable: true
     * @var \SengentoBV\ChronopostSdk\Soap\TrackingService\Structs\ChronoApiEvent[]
     */
     ...
     * @uses ChronoApiListEvents::setEvents()
     * @uses ChronoApiListEvents::setSkybillNumber()
     * @param \SengentoBV\ChronopostSdk\Soap\TrackingService\Structs\ChronoApiEvent[] $events
    ...
    public static function validateEventsForArrayConstraintsFromSetEvents(?array $values = []): string
    {
        ...
        foreach ($values as $listEventsEventsItem) {
            ...
            if (!$listEventsEventsItem instanceof \SengentoBV\ChronopostSdk\Soap\TrackingService\Structs\ChronoApiEvent) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The events property can only contain items of type \SengentoBV\ChronopostSdk\Soap\TrackingService\Structs\ChronoApiEvent, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \SengentoBV\ChronopostSdk\Soap\TrackingService\Structs\ChronoApiEvent $item
     * @return \SengentoBV\ChronopostSdk\Soap\TrackingService\Structs\ChronoApiListEvents
     */
    public function addToEvents(\SengentoBV\ChronopostSdk\Soap\TrackingService\Structs\ChronoApiEvent $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \SengentoBV\ChronopostSdk\Soap\TrackingService\Structs\ChronoApiEvent) {
            throw new InvalidArgumentException(sprintf('The events property can only contain items of type \SengentoBV\ChronopostSdk\Soap\TrackingService\Structs\ChronoApiEvent, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        $this->events[] = $item;
